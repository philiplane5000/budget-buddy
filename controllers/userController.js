...
module.exports = {
  ...
  createNewUser: function(req, res) {
    db.User.create({ uid: req.params.uid, budgets: BudgetSeed, articles: [] })
      .then(User => res.json(User))
      .catch(err => res.status(422).json(err));
  },
  ...
  getBudgetbyUserUid: function(req, res) {
    db.User.findOne({ uid: req.params.uid }, "budgets")
      .then(User => res.json(User))
      .catch(err => res.status(422).json(err));
  },

  getArticlesbyUserUid: function(req, res) {
    db.User.findOne({ uid: req.params.uid }, "articles")
      .then(User => res.json(User))
      .catch(err => res.status(422).json(err));
  },
  //RETRIEVE ALL USERS:
